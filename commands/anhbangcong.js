...
cron.schedule('30 1 * * *', async () => { // 2 giờ UTC là 9 giờ sáng theo giờ Việt Nam
  ...
  await processAndDistributeOtherTimesheets(bot, chatId);
});
...
const allowedGroupIds = [
  -1002230199552, -1002479414582, -1002449707024, -1002160116020, -1002259135527, -1002349272974, -1002312409314, -1002439441449, -1002178207739, -1002235474314, -1002186698265, -1002205826480,
  -1002311358141, -1002481836552, -1002245725621, -1002350493572, -1002300392959, -1002113921526, -1002243393101, -1002311651580
];

async function processAndDistributeTimesheets(bot, chatId, isToday) {
  ...
  try {
    ...
    for (const groupId of allowedGroupIds) {
      ...
      const groupName = await fetchGroupTitle(bot, groupId);
...
async function processAndDistributeOtherTimesheets(bot, chatId) {
...
async function fetchGroupTitle(bot, groupId) {
  try {
...
function initTimesheetCommands(bot) {
  // Chức năng tự động gửi hình ảnh vào 9h sáng mỗi ngày (theo giờ Việt Nam)
  cron.schedule('30 1 * * *', async () => { // 2 giờ UTC là 9 giờ sáng theo giờ Việt Nam
    const chatId = '-1002103270166';
    await processAndDistributeOtherTimesheets(bot, chatId);
...
module.exports = (bot) => {
  ...
  bot.onText(/\/homnaylan/, async (msg) => {
    const chatId = msg.chat.id;
    await processAndDistributeTimesheets(bot, chatId, true);
  });
